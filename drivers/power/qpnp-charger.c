...
#define HOT_TEMP_DEFAULT		520	/* stop charging */
#define COLD_TEMP_DEFAULT		0	/* stop charging */
...
static int huawei_hot_design_iusb_set(int mA)
{
	...
	if (global_chip != NULL) {
		if (qpnp_chg_is_usb_chg_plugged_in(global_chip)) {
			global_chip->usb_psy->get_property(
				global_chip->usb_psy,
				POWER_SUPPLY_PROP_CURRENT_MAX, &ret);
			if (ret.intval > 2) {
				ret.intval = ret.intval / 1000;
				ret.intval = ret.intval <= hot_design_current ?
					ret.intval : hot_design_current;
				rc = qpnp_chg_iusbmax_set(
					global_chip,ret.intval);
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		/* irq_handler does not allow call function that uses mutex,
...
#ifdef CONFIG_HUAWEI_KERNEL
			/* wake_lock to update the state of led,
			   prevents phone from entering sleep */
			wake_lock_timeout(&chip->led_wake_lock,
				msecs_to_jiffies(LED_CHECK_PERIOD_MS));
...
static void qpnp_chg_usb_low_power_work(struct work_struct *work)
{
	...
	while(usb_ma < USB_WALL_THRESHOLD_MA && usb_present)
	{
		...
		vchg_uv = get_vchg_averaged(chip, 16);
		msleep(200);
		pr_info("vbat_uv=%d, vusb_uv=%d, vchg_uv=%d, usb_present=%d, usb_ma=%d\n",
...
#ifdef CONFIG_HUAWEI_KERNEL
/* max fake value for decrease between two continuous values when charger is
   connected. modify the max fake value from 15 to 6 */
...
#ifdef CONFIG_HUAWEI_KERNEL
		if (chip->resuming_charging == true &&
			get_prop_charge_type(chip) == POWER_SUPPLY_CHARGE_TYPE_NONE)
			chip->resuming_charging = false;
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		if(last_soc < 0 || running_test_soc_enable ||
			soc < DEFAULT_CAPACITY) {
			last_soc = soc;
		} else if (charger_in && (last_soc > soc) &&
				(last_soc - soc < MAX_FAKE_REDUCE_VALUE)) {
...
{
	/* if the CUTOFF_BATTERY_LEVEL had been reported for 90s but the headset
	   is still on, power it off forcibly to avoid battery overdraw */
...
#endif /* CONFIG_CHARGER_BQ2419x || CONFIG_BATTERY_BQ27510 */
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
...
#ifdef CONFIG_HUAWEI_KERNEL
		/* check if the battery level is lower than cutoff level */
		poweroff_batt_level_check(chip, val);
#endif
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
...
static int hw_tm_warm_notification_zone(int temp,struct qpnp_chg_chip *chip)
{
	...
	pr_err("warm notification error, temp=%d \n", temp);
	return UNKNOW_ZONE;
}
...
======================================================================================*/
/* solve the problem when vbatt's voltage is over the vbatdet of WARM_HOT_ZONE,
   phone can not recover charge from HOT_HOT_ZONE to WARM_HOT_ZONE */
static void hw_tm_set_configure(enum hw_high_low_temp_configure_type zone,struct qpnp_chg_chip *chip)
{
	...
	if(bad_temp_flag ^ bad_temp) {
		...
		pr_info("bad_temp_flag=%d, qpnp_chg_charge_en=%d\n",
	...
	pr_debug("warm=%d, cool=%d, low=%d deciDegC, high=%d deciDegC, hot=%d, warm=%d, cool=%d, cold=%d\n",
		chip->bat_is_warm, chip->bat_is_cool, chip->adc_param.low_temp,
		chip->adc_param.high_temp, chip->hot_bat_decidegc,
		chip->warm_bat_decidegc, chip->cool_bat_decidegc,
...
#if defined(CONFIG_CHARGER_BQ2419x) || defined(CONFIG_BATTERY_BQ27510)
		if (bq_device != NULL) {
			bq2419x_set_enable_hz_mode_for_factory_test(bq_device,
				!(val->intval));
			break;
		}
#endif
...
#if defined(CONFIG_CHARGER_BQ2419x) || defined(CONFIG_BATTERY_BQ27510)
		if (bq_device != NULL) {
			bq2419x_set_enable_charger_for_factory_test(bq_device,
				val->intval);
			break;
		}
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		if ((chip->cool_bat_decidegc || chip->warm_bat_decidegc) &&
				(batt_data.warm_bat_decidegc ||
				 batt_data.cool_bat_decidegc)){
...
#ifdef CONFIG_HUAWEI_KERNEL
			of_property_read_u32(chip->dev->of_node, "usb-ocp-thr",
					&usb_ocp_thr);
			rc = qpnp_chg_masked_write(chip,
					chip->usb_chgpth_base + USB_OCP_THR,
					OCP_THR_MASK,
					usb_ocp_thr, 1);
			pr_info("%s: usb_ocp_thr = %x \n", __func__, usb_ocp_thr);
#else
			rc = qpnp_chg_masked_write(chip,
				chip->usb_chgpth_base + USB_OCP_THR,
				OCP_THR_MASK,
