...
/* used for bq27510 getting battery info */
struct qpnp_bms_chip *global_bms_chip = NULL;
#endif
...
#ifdef CONFIG_HUAWEI_KERNEL
		if (raw->last_good_ocv_uv > chip->ocv_low_threshold_uv
		    && raw->last_good_ocv_uv < chip->ocv_high_threshold_uv) {
			chip->estimate_new_ocv_flag = 1;
			pr_info("ocv_low_threshold_uv=%d, ocv_high_threshold_uv=%d, last_good_ocv_uv=%d\n",
	...
	if (new_calculated_soc == CAPACITY_99 && is_battery_charging(chip)) {
		...
		if(ibat_ua > DELTA_I_REPORT_100 && ibat_ua < 0) {
			...
			if(chip->fake_soc_count == SAMPLE_COUNTER) {
				...
				chip->last_ocv_uv = find_ocv_for_pc(chip,
					batt_temp,
					find_pc_for_soc(chip,
						&params, new_calculated_soc));
...
int64_t get_battery_id_from_bms(void)
{
	...
	if (global_bms_chip != NULL) {
		...
		pr_err("global_bms_chip did not init\n");
	}
