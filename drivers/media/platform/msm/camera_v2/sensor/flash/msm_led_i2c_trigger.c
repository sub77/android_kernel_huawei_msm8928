	...
	void *data)
{
	...
	if((TEMPERATUE_ABNORMAL == led_temperature)
		&& (MSM_CAMERA_LED_TORCH_POWER_NORMAL != cfg->cfgtype)) {
	...
	case MSM_CAMERA_LED_TORCH_LOW_HIGH:
		if (fctrl->func_tbl->torch_led_on){
			/* have to sleep to solve the flash problem of torch app */
			msleep(100);
...
* NOTE: this function must be called before register is read and write
***************************************************************************/
