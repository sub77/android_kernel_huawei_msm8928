	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_CLK:
			...
			rc = msm_cam_clk_enable(s_ctrl->sensordata->power_info.dev,
				...
				s_ctrl->sensordata->power_info.clk_info_size,
				1);
			CDBG("%s powerup clock num %d", __func__, s_ctrl->sensordata->power_info.clk_info_size);
			if (rc < 0) {
	...
	for (index = 0; index < power_setting_array->size; index++){
		...
		case SENSOR_CLK:
			msm_cam_clk_enable(s_ctrl->sensordata->power_info.dev,
				...
				s_ctrl->sensordata->power_info.clk_info_size,
				0);
			CDBG("%s powerdown clock num %d\n", __func__, s_ctrl->sensordata->power_info.clk_info_size);
			break;
		case SENSOR_GPIO:
			gpio_set_value_cansleep(
				data->gpio_conf->gpio_num_info->gpio_num
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_CLK:
			msm_cam_clk_enable(s_ctrl->sensordata->power_info.dev,
				...
				s_ctrl->sensordata->power_info.clk_info_size,
				0);
			CDBG("%s powerdown clock num %d", __func__, s_ctrl->sensordata->power_info.clk_info_size);
			break;
		case SENSOR_GPIO:
			if (power_setting->seq_val >= SENSOR_GPIO_MAX ||
				!data->gpio_conf->gpio_num_info) {
