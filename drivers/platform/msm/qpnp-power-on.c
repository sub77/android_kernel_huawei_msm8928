...
#ifdef CONFIG_HUAWEI_KERNEL
#define QPNP_PON_POFF_REASON1(base)	(base + 0xD)
#define QPNP_PON_POFF_REASON2(base)	(base + 0xE)
#endif
...
static int __devinit qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	rc = qpnp_pon_masked_write(pon, PON_SPARE_REG,
		PON_SPARE_WRITE_MASK, PON_SPARE_REG_EN);
	if (rc) {
		dev_err(&spmi->dev, "Unable to write PON spare reg\n");
		return rc;
	}
#endif
